...
async function createGame() {
  ...
  if (!nameInput.value) {
    loading.value = false
    window.alert('Es wurde kein Raumname eingegeben.')

    return
  }

  const game = await client.from('games').select().eq('name', nameInput.value)

  if (game?.data?.length > 0) {
    loading.value = false
    window.alert('Es existiert bereits ein Spiel mit diesem Raumnamen.')
  ...
  const newGame = await client
    .from('games')
    .insert({ name: nameInput.value })
    .select()

  if (newGame?.data[0].id != null) {
    loading.value = false

    emit('create', newGame.data[0].id)
  }
}
</script>
...
<template>
  <div class="flex flex-col text-center">
    <!-- <h2 class="h3 -mt-5 mb-6">
      Create Game
    </h2>
    <input v-model="nameInput" type="text" placeholder="Raumname">
    ...
    <button v-else :class="{ 'opacity-50 pointer-events-none': !nameInput }" @click="createGame">
      – Erstellen –
    </button>
  </div>
</template>
